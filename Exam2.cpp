...
public:
    ...
    static void removeUnqualifiedEmployees(double reqYears, double reqSalary, vector<Employee> &employees){
        ...
        while(i < employees.size()){
            if(employees[i].getYearsInCompany() >= reqYears || employees[i].getHourlySalary() <= reqSalary){
    ...
    static bool hasDuplicates(const vector<Employee> &employees){
        ...
        while(i < employees.size()){
            for(int j = i+1; j < employees.size(); j++){
                if(employees[j].equals(employees[i])){
                    return true;
                }
            }
        i++;
    ...
    static Employee lastOccurrenceOfAnEmployee(double reqYears, Position reqPosition, const vector<Employee> &employees){
        ...
        while(j < employees.size()){
            if(employees[j].getPosition() == reqPosition && employees[j].getYearsInCompany() >= reqYears){
                last = j;
            }
            j++;
        }
    ...
    static double lowestSalaryPerPosition(Position p, const vector<Employee> &employees){
        double minSal = 100000;
        bool exist = false;
        for(Employee a : employees){
            if(a.getPosition() == p && a.getHourlySalary() < minSal){
                minSal = a.getHourlySalary();
                exist = true;
            }
        }
    if(!exist){
    ...
    static int highestScoreIndex(const vector<Employee> &employees){
      int score = 0;
        ...
        while(i < employees.size()){
           if((employees[i].getHourlySalary() * employees[i].getYearsInCompany() * yearly) > (employees[score].getHourlySalary() * employees[score].getYearsInCompany() * yearly)){
                score = i;
           }
           i++;
        }
    ...
    static double avgSalaryperPosition(Position p, const vector<Employee> &employees){
        ...
        bool exist = false;
        while(i < employees.size()){
            if(employees[i].getPosition() == p ){
                avgSal += employees[i].getHourlySalary();
                exist = true;
                j++;
            }
        }
 ...
 * Otherwise return false (Hint: Use the given equals method).
 */
bool Employee::hasDuplicates(const vector<Employee> &employees)
{
    // YOUR CODE HERE
